...
function formatProductType(productType: string) {
  switch (productType) {
    ...
    case "hosting_pack":
      return "Hosting Pack";
    default:
      return slugToTitle(productType.replace(/[_-]+/g, "-"));
  }
}
...
function ProductDetailPage() {
  ...
  }, [existingLine?.quantity, product?.id, selectedVariant?.id]);
  ...
  function handleAddToCart() {
    ...
    if (existingLine) {
      setItemQuantity(activeCartTarget!, selectedQuantity);
    } else {
      addItem(input);
    ...
    toast.success(`${product?.name ?? "Item"} added to your order`, {
      description: selectedVariant ? `${selectedVariant.name} saved with live USD pricing.` : "Saved with live catalog pricing.",
    });
      ...
      <div className="page-shell grid gap-8 lg:grid-cols-[1.05fr,0.95fr] lg:gap-10">
        ...
        <div className="space-y-5 sm:space-y-6">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className="inline-flex items-center rounded-full border border-[var(--vf-border-soft)] bg-[var(--vf-surface-elevated)] px-3 py-1 text-[11px] font-bold uppercase tracking-[0.14em] text-[var(--vf-secondary)]">
              {formatProductType(product.productType)}
            </span>
          </div>

          <div>
            <p className="text-xs font-bold uppercase tracking-[0.2em] text-[var(--vf-primary)]">Menu Detail</p>
            <h1 className="heading-display mt-2 text-4xl font-bold sm:text-5xl">{product.name}</h1>
            <p className="mt-3 text-sm font-medium uppercase tracking-[0.16em] text-[var(--vf-text-soft)]">
              {product.servingSize}
            </p>
          </div>

          <div className="rounded-[var(--vf-radius-lg)] border border-[var(--vf-border-soft)] bg-[var(--vf-surface-card)] p-5 shadow-[var(--vf-shadow-soft)]">
            <p className="text-xs font-bold uppercase tracking-[0.18em] text-[var(--vf-text-soft)]">Price</p>
            ...
            {selectedVariant?.salePrice !== null && selectedVariant?.salePrice !== undefined ? (
              <p className="mt-1 text-sm text-[var(--vf-text-muted)]">Sale pricing is reflected in the selected variant.</p>
            ) : null}
          </div>

          <div className="text-base leading-8 text-soft">
            <p>{product.shortDescription}</p>
            {product.description && product.description !== product.shortDescription ? <p className="mt-4">{product.description}</p> : null}
          </div>
                    ...
                    >
                      ...
                      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                        <div>
                          ...
                          <p className="mt-1 text-sm text-soft">{variant.portionLabel || variant.description || "Standard serving"}</p>
                        </div>
          ...
          <div className="card-surface flex flex-col gap-4 p-5 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
            ...
            >
              {isSoldOut ? "Unavailable" : existingLine ? "Update Order" : "Add to Order"}
            </button>
            <QuantityStepper value={selectedQuantity} onChange={handleQuantityChange} />
          </div>
        </div>
      </div>
    </main>
    ...
    <main className="section-gap">
      <div className="page-shell">
        <article className="card-surface space-y-4 p-6 sm:p-8">
          ...
          <p className="text-soft">{message ?? "Unable to load the latest product details at the moment. Please try again."}</p>
